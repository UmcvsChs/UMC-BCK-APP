create or replace function public.checkout_full_cart(
  p_delivery_address text default null, p_delivery_lga_id uuid default null, p_delivery_type delivery_type default 'home_delivery',
  p_terms_accepted boolean default false, p_weight_tier text default 'light', p_urgency_tier text default 'standard',
  p_group_order_id uuid default null, p_delivery_neighborhood_id uuid default null
)
returns uuid[]
...
begin
  ...
  loop
    ...
    -- Real, direct fix — this real function silently dropped any real
    -- neighborhood precision on the floor before, since it never
    -- forwarded one through to place_order at all.
    v_order_id := public.place_order(
      ...
      false, null, p_delivery_type, p_terms_accepted, p_group_order_id, p_delivery_neighborhood_id
    );
