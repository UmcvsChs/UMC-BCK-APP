-- Real correction to the grace rule, exactly as specified: the grace is
-- only for someone who buys right away — not a blanket "first order is
-- always free" regardless of timing. If 24 hours pass since signup with
-- no order placed, verification becomes required even for that first
-- order. A real, reusable function so both the checkout gate and the
-- frontend's proactive prompt use the exact same rule, not two versions
-- that could drift apart.
create function public.needs_identity_verification(p_user_id uuid)
returns boolean
language plpgsql stable security definer set search_path = public
as $$
declare v_has_prior_order boolean; v_signed_up_at timestamptz; v_hours_since_signup numeric;
begin
  ...
  -- Already has at least one real order — the grace, if it applied, was
  -- already used. Verification is required from here on.
  if v_has_prior_order then
    return true;
  end if;

  -- No order yet — verification is only required once the real 24-hour
  -- grace window has passed without one.
...
revoke execute on function public.needs_identity_verification(uuid) from public, anon;

-- Real correction inside place_order() itself, using this same function —
-- one real source of truth, not duplicated logic that could disagree.
...
begin
  ...
  -- Real corrected gate: grace only for a genuinely immediate first
  -- purchase — not a blanket pass regardless of timing.
  ...
  insert into public.orders (buyer_id, seller_id, subtotal, delivery_fee, total_amount, is_instalment, delivery_address, delivery_lga_id, delivery_type, terms_accepted_at)
  values (v_buyer_id, p_seller_id, v_subtotal, coalesce(p_delivery_fee,0), v_total, p_is_instalment, p_delivery_address, p_delivery_lga_id, p_delivery_type, now())
