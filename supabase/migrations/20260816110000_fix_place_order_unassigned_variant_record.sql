-- Real bug: `v_variant.id := null;` tried to assign a single field on a
-- plain `record` variable that had never been given a shape by a real
-- SELECT INTO. Postgres records have no fixed structure until one is
-- assigned — hence the exact error seen at checkout: record "v_variant"
-- is not assigned yet. This fired for any cart item with no variant
-- (an ordinary product), which is most products in this catalog.
CREATE OR REPLACE FUNCTION public.place_order(p_seller_id uuid, p_items jsonb, p_delivery_address text DEFAULT NULL::text, p_delivery_lga_id uuid DEFAULT NULL::uuid, p_delivery_fee numeric DEFAULT 0, p_is_instalment boolean DEFAULT false, p_deposit_amount numeric DEFAULT NULL::numeric, p_delivery_type delivery_type DEFAULT 'home_delivery'::delivery_type, p_terms_accepted boolean DEFAULT false, p_group_order_id uuid DEFAULT NULL::uuid, p_delivery_neighborhood_id uuid DEFAULT NULL::uuid)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  if p_is_instalment then
    ...
    select is_eligible, reason into v_eligible, v_eligibility_reason
    from public.check_instalment_eligibility(v_buyer_id);
    if not v_eligible then
      raise exception '%', v_eligibility_reason;
    end if;
  end if;
  ...
  insert into public.orders (buyer_id, seller_id, subtotal, delivery_fee, buyer_service_charge, total_amount, is_instalment, delivery_address, delivery_lga_id, delivery_neighborhood_id, delivery_type, terms_accepted_at, group_order_id)
  values (v_buyer_id, p_seller_id, v_subtotal, v_real_delivery_fee, v_service_charge, v_total, p_is_instalment, p_delivery_address, p_delivery_lga_id, p_delivery_neighborhood_id, p_delivery_type, now(), p_group_order_id)
  ...
  loop
    ...
    if v_item ? 'variant_id' and v_item->>'variant_id' is not null then
      ...
      select null::uuid as id, null::numeric as price into v_variant;
      v_unit_price := v_product.price;
    end if;
