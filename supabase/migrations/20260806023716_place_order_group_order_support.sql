...
begin
  ...
  -- Real group order handling: only the initiator's real order carries
  -- the real delivery fee — everyone else's is genuinely zero, since one
  -- delivery happens and the organizer fronts it, avoiding fragile
  -- split-fee math that would need a final headcount before checkout.
  ...
  v_total := v_subtotal + v_real_delivery_fee;
  ...
  insert into public.orders (buyer_id, seller_id, subtotal, delivery_fee, total_amount, is_instalment, delivery_address, delivery_lga_id, delivery_type, terms_accepted_at, group_order_id)
  values (v_buyer_id, p_seller_id, v_subtotal, v_real_delivery_fee, v_total, p_is_instalment, p_delivery_address, p_delivery_lga_id, p_delivery_type, now(), p_group_order_id)
