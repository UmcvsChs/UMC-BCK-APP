-- p_items now accepts optional variant_id, addon_ids, and contributor_name per
-- line: [{"product_id":"...","quantity":2,"variant_id":"...","addon_ids":["..."],"contributor_name":"Amina"}]
-- All three are optional — a plain grocery order with none of them behaves
-- exactly as before.
...
begin
  ...
  -- Pass 1: validate and compute subtotal
  ...
  -- Pass 2: insert line items (and their addon selections) now that the order exists
  for v_item in select * from jsonb_array_elements(p_items)
  loop
    ...
    insert into public.order_items (order_id, product_id, product_variant_id, quantity, unit_price, line_total, contributor_name)
    values (v_order_id, v_product.id, v_variant.id, (v_item->>'quantity')::integer, v_unit_price, v_line_total,
