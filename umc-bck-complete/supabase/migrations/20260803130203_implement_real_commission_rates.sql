-- Real commission rates, as explicitly decided via business consulting in
-- the project's first session and committed to at the time. Only hubs with
-- a confirmed, explicit rate get one — general_marketplace and
-- pharma_medical were never given a rate in that consulting conversation,
-- so they stay at 0% rather than have a rate invented for them now.
create function public.get_commission_rate(p_hub text)
returns numeric
