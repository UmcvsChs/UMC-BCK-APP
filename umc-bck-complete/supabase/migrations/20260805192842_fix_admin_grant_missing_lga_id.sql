-- Real bug caught while granting the actual first admin: sellers and
-- delivery_agents both require a real lga_id, which this function never
-- provided. Fixed with a real default (the first genuine Kaduna LGA on
-- record), so granting the rest of the team doesn't hit the same wall.
...
begin
  ...
  select l.id into v_default_lga_id
  from public.local_government_areas l join public.states s on s.id = l.state_id
  where s.name = 'Kaduna' order by l.name limit 1;
  ...
  if not exists (select 1 from public.sellers where user_id = v_target_user_id) then
    ...
    values (v_target_user_id, coalesce(v_target_name, 'Test') || '''s Store', 'general_marketplace', 'individual', 'approved', true, v_default_lga_id);
  ...
  if not exists (select 1 from public.delivery_agents where user_id = v_target_user_id) then
    insert into public.delivery_agents (user_id, vehicle_type, verification_status, is_online, lga_id)
    values (v_target_user_id, 'motorcycle', 'approved', true, v_default_lga_id);
