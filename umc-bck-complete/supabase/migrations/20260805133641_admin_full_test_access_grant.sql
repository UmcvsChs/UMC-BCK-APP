-- Real infrastructure for genuine multi-role testing, matching the sister
-- app's pattern: one real account, made admin, with real (not fake or
-- impersonated) seller/delivery-agent/repairer records under that same
-- identity, pre-approved since the admin is authorizing their own test
-- access. This is NOT impersonation of anyone else's data — every role the
-- test account sees is genuinely its own, real, and auditable.
create function public.admin_grant_full_test_access(p_phone text)
returns text
...
begin
  ...
  if not exists (select 1 from public.repairers where user_id = v_target_user_id) then
    insert into public.repairers (user_id, business_name, verification_status)
    values (v_target_user_id, coalesce(v_target_name, 'Test') || '''s Repair Shop', 'approved');
...
revoke execute on function public.admin_grant_full_test_access(text) from public, anon;