-- Real fix for a genuine gap named in the handover document: the buyer
-- must be the one confirming receipt before funds settle — not the
-- delivery agent self-attesting on the buyer's behalf, which is what the
-- UI text claimed while the actual code allowed anyone involved to call
-- this. Restricted to the real buyer or admin only.
...
begin
  ...
  -- Real restriction: only the genuine buyer can confirm their own
  -- delivery, or admin for legitimate dispute-resolution override. The
  -- agent and seller no longer can — they were never the ones the
  -- handover document says should be confirming this.
  if v_caller <> v_order.buyer_id and not v_is_admin then
    raise exception 'Only the buyer who placed this order can confirm receipt — or admin, for dispute resolution';
  end if;

  if v_order.status not in ('confirmed','assigned','preparing') then
