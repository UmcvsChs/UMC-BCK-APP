-- Critical fix, found while implementing commission: finalize_wallet_hold()
-- only releases the buyer's held funds — it never credits anyone. No
-- trigger exists to pay the seller either. This means mark_order_delivered()
-- has never actually paid a seller for a regular order, in the real
-- database, for any order. Confirmed no real damage has occurred (zero
-- real sellers/orders exist in this project yet), but this needed fixing
-- before a single real transaction could go through broken.
...
begin
  ...
  -- Real commission, matching the rates explicitly decided in the
  -- project's original business-model consulting session.
  v_commission_rate := public.get_commission_rate(v_order.primary_hub);
  v_commission_amount := round(v_total_held * v_commission_rate, 2);
  v_seller_payout := v_total_held - v_commission_amount;

  -- Release the buyer's hold, then actually pay the seller — this credit
