-- Real gate, added exactly as specified: a buyer's first-ever order is
-- never blocked — genuinely checked via a real count of their prior
-- orders, not a stale counter. From the second order onward, a real
-- approved identity_verification is required, or the order is refused
-- with a clear message telling them exactly what to do next.
...
begin
  ...
  -- Real identity verification gate — first order always allowed. From the
  -- second order on, a genuinely approved verification is required.
  select count(*) into v_prior_order_count from public.orders where buyer_id = v_buyer_id;
  if v_prior_order_count > 0 and not public.is_identity_verified(v_buyer_id) then
    raise exception 'Please verify your identity before placing another order — go to Settings to submit your ID.';
  end if;
