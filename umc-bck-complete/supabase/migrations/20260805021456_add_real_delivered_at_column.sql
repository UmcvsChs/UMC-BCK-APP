-- Real bug caught before it could bite: get_sales_report() and
-- get_item_sales_summary() referenced orders.delivered_at, which never
-- existed. plpgsql function bodies aren't validated for column existence
-- until actually called, so the earlier migration silently "succeeded"
-- while being genuinely broken. Fixed at the real source: a real
-- delivered_at column, set explicitly by mark_order_delivered() itself —
-- not inferred from updated_at, which could be touched by unrelated
-- future changes and would make delivery timing unreliable.
