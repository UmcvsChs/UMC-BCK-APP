...
import InstalmentPanel from '../components/InstalmentPanel'
...
export default function ProductDetail() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [{ data: p, error: pErr }, { data: v }, { data: a }, { data: w }] = await Promise.all([
        supabase.from('products').select('*, sellers(return_policy, primary_hub, wholesale_min_quantity, instalment_opt_in)').eq('id', productId).single(),
  ...
  return (
    <div className="p-4 max-w-lg mx-auto">
      ...
      {product.sellers?.instalment_opt_in && <InstalmentPanel product={product} sellerId={product.seller_id} />}
